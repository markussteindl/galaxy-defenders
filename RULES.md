...
2. **Defend roll**: if the defender has an energy shield, it **must** first cancel hits, 1 shield *point* per hit (a "2" token covers two hits: it becomes a "1" on the first, is removed on the second). Then roll 1 BLUE die per remaining hit; each shield result cancels a hit. Each uncancelled hit = 1 wound.
...
- **Wounds/HP**: track with wound tokens; HP can't exceed the printed maximum.
...
- Weapon ranks: **GREEN** standard, **BLUE** improved, **RED** alien. New weapons replace one of the agent's two current weapons (may be refused → returns to HQ/box).
